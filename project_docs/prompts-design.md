...
embed_status() -> EmbedStatus
    ...
    //   model_id: string, model_size_mb: number, error?: string }
...
set_embed_enabled(enabled: bool) -> null
    // Persisted app-config toggle (appconfig.rs); "ready" + enabled = hybrid on.
```
...
- **Semantic (opt-in):** fastembed-rs (ONNX, CPU) with a small, mature retrieval model (builder
  ...
  MTEB-retrieval-per-MB; e.g. bge-small-en-v1.5 class). Model downloads to `~/.ccdeck/models/`
...
## Backups migration — de-contaminate `~/.claude`

Shipped installs write session-edit backups to `~/.claude/.ccstudio-backups`. That was
contamination of Claude Code's directory; backups move to `~/.ccdeck/backups/`.

- On startup (in `run()`), if the legacy dir exists and `~/.ccdeck/backups` does not: rename
  (same filesystem — it's the same home dir) and leave nothing behind. If both exist (a half
  migration or downgrade-then-upgrade), merge legacy session dirs that don't collide, prefer the
  new location on collision, and remove the legacy dir only when emptied.
- Migration failure is non-fatal: the app must still boot (backups are a safety net, not core
  data); log and fall back to reading whichever dir has the file.
